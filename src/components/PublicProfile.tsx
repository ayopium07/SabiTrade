...
export default function PublicProfile() {
  ...
  const renderInsightCard = (post: any, isLikeView = false, originalAuthor?: any) => {
    ...
    return (
      <div 
        ...
        className="mb-4 rounded-2xl p-5 border border-border bg-bg-surface shadow-md hover:border-brand-primary/30 transition-all cursor-pointer group"
      >
        {isLikeView && (
        ...
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center justify-between mb-1">
              ...
              <span className="text-text-secondary text-xs font-medium whitespace-nowrap ml-2 bg-bg-raised px-2 py-1 rounded-lg">
            ...
            <div className="flex items-center gap-6 mt-5 pt-4 border-t border-border text-xs font-bold text-text-secondary">
              <button className="flex items-center gap-2 hover:text-text-primary transition-colors bg-bg-raised px-3 py-1.5 rounded-lg">
                <MessageSquare className="h-4 w-4" />
                <span>{post.repliesCount || 0}</span>
              </button>
              <button 
                onClick={(e) => { e.stopPropagation(); retweetPost(post.id, currentUserId); }}
                className={`flex items-center gap-2 transition-colors px-3 py-1.5 rounded-lg ${isRetweeted ? 'text-gain bg-gain/10' : 'hover:text-gain hover:bg-gain/10 bg-bg-raised'}`}
              >
              ...
              <button 
                ...
                className={`flex items-center gap-2 transition-colors px-3 py-1.5 rounded-lg ${isLiked ? 'text-brand-primary bg-brand-primary/10' : 'hover:text-brand-primary hover:bg-brand-primary/10 bg-bg-raised'}`}
              >
                <ThumbsUp className={`h-4 w-4 ${isLiked ? 'fill-brand-primary' : ''}`} />
                <span>{post.likes.length}</span>
              </button>
              <button className="flex items-center justify-center hover:text-[#B275FF] hover:bg-[#B275FF]/10 transition-colors ml-auto bg-bg-raised h-8 w-8 rounded-lg">
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6 pb-12">
      ...
      <div className="rounded-[2rem] p-6 sm:p-8 border border-border bg-bg-surface relative overflow-hidden shadow-lg">
        ...
        <div className="flex flex-col md:flex-row gap-8 relative z-10">
          ...
          <div className="flex-1 flex flex-col justify-between">
            ...
            <div className="grid grid-cols-3 gap-3 sm:gap-4">
              <div className="bg-bg-raised border border-border rounded-2xl p-4 flex flex-col justify-center shadow-sm">
      ...
      <div className="flex p-1.5 bg-bg-raised border border-border rounded-2xl shadow-sm">
        ...
        {activeTab === 'replies' && (
          userReplies.length === 0 ? (
            ...
            userReplies.map(reply => (
              <div key={reply.id} className="mb-4 rounded-2xl p-5 border border-border bg-bg-surface hover:border-brand-primary/30 transition-all group">
                <div className="text-[10px] uppercase tracking-widest font-bold text-text-secondary mb-3 flex items-center gap-1.5 bg-bg-raised border border-border w-fit px-2 py-1 rounded-md">
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-[14px] text-text-primary font-medium leading-relaxed bg-bg-raised p-3 rounded-xl border border-border">
                      {reply.content}
                    </p>
                  </div>
                </div>
